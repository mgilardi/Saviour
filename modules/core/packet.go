...
const (
	// MODULEPACKET is the packet module name constant
	MODULEPACKET = "Packet"
)
...
func loadDataPacket(buf []byte) (bool, DataPacket) {
	...
	if err != nil {
		Logger(err.Error(), PACKAGE+"."+MODULEPACKET+".loadDataPacket", ERROR)
		valid = false
	}
...
func sanitizePacket(packet DataPacket) DataPacket {
	...
	if err != nil {
		for _, err := range err.(validator.ValidationErrors) {
			switch err.Field() {
			case "User":
				Logger("PacketValidation::UserField", PACKAGE+"."+MODULEPACKET+".sanitizePacket", WARN)
				Logger("Sanitizing::"+packet.Login.User, PACKAGE+"."+MODULEPACKET+"sanitizePacket", MSG)
				regex := regexp.MustCompile("[^a-zA-Z0-9]")
				packet.Login.User = regex.ReplaceAllString(packet.Login.User, "")
				packet.Login.User = checkDataSize(packet.Login.User, 45)
				Logger("Sanitized::"+packet.Login.User, PACKAGE+"."+MODULEPACKET+".sanitizePacket", MSG)
			case "Pass":
				Logger("PacketValidation::PassField", PACKAGE+"."+MODULEPACKET+".sanitizePacket", WARN)
				Logger("Sanitizing::"+packet.Login.Pass, PACKAGE+"."+MODULEPACKET+"sanitizePacket", MSG)
				regex := regexp.MustCompile("[^a-zA-Z0-9]")
				packet.Login.Pass = regex.ReplaceAllString(packet.Login.Pass, "")
				packet.Login.Pass = checkDataSize(packet.Login.Pass, 45)
				Logger("Sanitized::"+packet.Login.Pass, PACKAGE+"."+MODULEPACKET+".sanitizePacket", MSG)
			case "Email":
				...
				Logger("Sanitized::"+packet.Login.Email, PACKAGE+"."+MODULEPACKET+".sanitizePacket", MSG)
			case "Username":
				Logger("PacketValidation::UsernameField", PACKAGE+"."+MODULEPACKET+".sanitizePacket", WARN)
				Logger("Sanitizing::"+packet.Saviour.Username, PACKAGE+"."+MODULEPACKET+".sanitizePacket", MSG)
				regex := regexp.MustCompile("[^a-zA-Z0-9]")
				...
				Logger("Sanitized::"+packet.Saviour.Username, PACKAGE+"."+MODULEPACKET+"sanitizePacket", MSG)
			case "Token":
				Logger("PacketValidation::TokenField", PACKAGE+"."+MODULEPACKET+".sanitizePacket", WARN)
				Logger("Sanitizing::"+packet.Saviour.Token, PACKAGE+"."+MODULEPACKET+".sanitizePacket", MSG)
				regex := regexp.MustCompile(`[^A-Za-z0-9+-_\/=]`)
				...
				Logger("Sanitized::"+packet.Saviour.Token, PACKAGE+"."+MODULEPACKET+".sanitizePacket", MSG)
			case "Message":
				Logger("PacketValidation::MessageField", PACKAGE+"."+MODULEPACKET+".sanitizePacket", WARN)
				Logger("Sanitizing::"+packet.Saviour.Message, PACKAGE+"."+MODULEPACKET+".sanitizePacket", MSG)
				regex := regexp.MustCompile(`[^A-Za-z0-9+-:_\/=]`)
				...
				Logger("Sanitized::"+packet.Saviour.Message, PACKAGE+"."+MODULEPACKET+".sanitizePacket", MSG)
			case "Status":
				Logger("PacketValidation::StatusField", PACKAGE+"."+MODULEPACKET+".sanitizePacket", WARN)
